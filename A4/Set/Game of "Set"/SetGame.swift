...
struct SetGame {
    ...
    private static let numberOfTrinitiesOfCardsAtTheStartOfTheGame = 4
    ...
    private var numberOfSelectedCards: Int {
        cardsOnTheTable.filter({$0.isSelected}).count
    }
    ...
    init() {
        ...
        for _ in 0..<SetGame.numberOfTrinitiesOfCardsAtTheStartOfTheGame {
            dealThreeMoreCards()
        }
    }
    ...
    /// Adds three more cards on the table and removes them from the deck. Also decreases 'score' if there was a Set in previous set of cards on the table
    ...
    /// Deselects and unmatches all the cards on the table
    ...
    /// Discards matched cards from the table and puts them into discard pile
    private mutating func discardMatchedCards() {
        discardPile.append(contentsOf: cardsOnTheTable.filter({ $0.isMatched == true }))
        
        for index in discardPile.indices {
            discardPile[index].isMatched = nil
            discardPile[index].isSelected = false
            discardPile[index].isHighlighted = false
        }
    ...
    /// Represents how many shapes there are on the card
    ...
    /// Represents the type of shape on the card
    ...
    /// Represent the shading of the shape on the card
